...
public class OSAuthenticator {
	 ...
	 * @param perspective
	 *            the network facing perspective
	 * @param config
	...
	public static void reAuthenticate() {
		...
		LOG.info("Re-Authenticating session due to expired Token or invalid response");
		...
		case V3:
		default: {
			LOG.info("Re-Auth - Keystone Version -> " + session.getAuthVersion());
	...
	private static OSClientV3 authenticateV3(KeystoneAuth auth, SessionInfo info, Config config) {
		if (auth == null) {
			LOG.error("**** KeystongAuth is null.");
		} else if (auth.getType() == null) {
			LOG.error("**** AuthType is null.");
		}
		if (Type.TOKENLESS.equals(auth.getType())) {
		...
		if (Type.CREDENTIALS.equals(auth.getType())) {
